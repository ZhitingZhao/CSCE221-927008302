...
*****************************************************/
    ...
    int max_depth (BinaryNode *t ) const
    {
        ...
        if (t == nullptr){
            ...
            if (max_depth(t->left) > max_depth (t->right)){
    ...
    int diameter (BinaryNode *t ) const
    {
        ...
        if (t == nullptr){
            ...
            int l = max_depth(t->left);
            int r = max_depth(t->right);
            int dia = l + r + 1;
            int dial = diameter(t->left);
            int diar = diameter(t->right);
            if (dia > dial && dia > diar){
                return dia;
            }
            else if (dial > diar){
                return dial;
    ...
    void LCA(BinaryNode *t, int x, int y, ostream & out) const
    {
        ...
        if (t->element == x || t->element == y){
            ...
            bool find = false;
            vector<int> wayX = WayToNode(t, x);
            vector<int> wayY = WayToNode(t, y);
        
            cout << "size:" << wayX.size() << endl;
            cout << "last:" << wayX.at(wayX.size() - 1) << endl;
        
        
            for (unsigned long i = wayX.size() - 1; i >= 0 && !find; i--){
                for (unsigned long  j = 0; j < wayY.size() && !find; j++){
                    if (wayX.at(i) == wayY.at(j)){
                        find = true;
                        out << wayX.at(i) << endl;
                    }
                }
    ...
    vector<int> WayToNode(BinaryNode *t, int a)const
    {
        vector<int> r;
        while (t->element != a){
            if (t->element > a){
                r.push_back(t->element);
                t = t->left;
            }
            else{
                r.push_back(t->element);
